...
void Hym::IrradianceField::Init(int probesX, int probesY, int probesZ, int raysPerProbe, Scene& scene)
{
	...
	L.probeStep = (maxScene - minScene + glm::vec3(1.3f)) / (glm::vec3(L.probeCounts) - glm::vec3(1));
	...
	createPSOBorderOp(onesBorder_IrradiancePass, irradianceTex, "Ones To Border Irradiance PSO", SHADER_RES "/borderOperations_cs.hlsl", irrSL.c_str(), "mainWriteOnesToBorder");
	createPSOBorderOp(copyBorder_WeightPass, weightTex, "Copy Border Weight PSO", SHADER_RES "/borderOperations_cs.hlsl", depthSL.c_str(), "mainDuplicateProbeEdges");
	createPSOBorderOp(onesBorder_WeightPass, weightTex, "Ones To Border Weight PSO", SHADER_RES "/borderOperations_cs.hlsl", depthSL.c_str(), "mainWriteOnesToBorder");
}

void Hym::IrradianceField::createComputeRaysPSO(Hym::Scene& scene)
{
	SamplerDesc sdesc
	{
